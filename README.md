...
반응변수가 범주형 변수이므로 로지스틱회귀모형을 사용하겠다. 대부분 사람들의 혈액검사 항목의 수치가 정상에 속하므로 기본적으로 모델이 예측한 비정상일 확률은 낮을 것이다. 따라서 이를 반영하고자 타켓이 1(비정상)일 확률이 0.2보다 높으면 비정상으로 예측했다. 아래는 반응변수에 따른 훈련 에 대한 모형의 $R^2$와 F1-score를 나타낸 표이다. 
